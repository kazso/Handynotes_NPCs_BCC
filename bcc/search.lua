...
-- Helper function to prevent "SetFont" crashes on modern clients
local function SafeSetFont(object, fontName, fontSize, flags)
    local fontPath = LSM:Fetch("font", fontName) or "Fonts\\FRIZQT__.TTF"
    local size = fontSize or 12
    if size <= 0 then size = 12 end
    object:SetFont(fontPath, size, flags or "")
end
...
function Search:UpdateSettings()
	local currentLINE_HEIGHT = LINE_HEIGHT * Main.db.profile.search.scaleFactor
	...
	window:SetHeight((NUM_LINES + 1) * currentLINE_HEIGHT + 8)
	window.header:SetHeight(currentLINE_HEIGHT)
	window.header:SetWidth(window:GetWidth()-10)
    
    SafeSetFont(window.header.ltext, Main.db.profile.search.header.font, Main.db.profile.search.header.fontsize)
    SafeSetFont(window.header.rtext, Main.db.profile.search.header.font, Main.db.profile.search.header.fontsize)

	window.footer:SetHeight(currentLINE_HEIGHT)
	window.footer:SetWidth(window:GetWidth()-10)
    SafeSetFont(window.footer.rtext, Main.db.profile.search.footer.font, Main.db.profile.search.footer.fontsize)

	window.f:SetScript("OnVerticalScroll", function(self, offset) FauxScrollFrame_OnVerticalScroll(self, offset, currentLINE_HEIGHT, update) end)
	
    SafeSetFont(window.editbox, Main.db.profile.search.search.font, Main.db.profile.search.search.fontsize)
	window.editbox:SetWidth(window:GetWidth()-20)
	window.editbox:SetHeight((select(2, window.editbox:GetFont()) or 12)+8)
	...
	for i = 1, NUM_LINES do
        ...
        SafeSetFont(frame.ltext, Main.db.profile.search.list.font, Main.db.profile.search.list.fontsize)
        SafeSetFont(frame.rtext, Main.db.profile.search.list.font, Main.db.profile.search.list.fontsize)
		frame:SetHeight(currentLINE_HEIGHT)
...
function Search:OnInitialize()
	...
	local currentLINE_HEIGHT = LINE_HEIGHT * Main.db.profile.search.scaleFactor
	local anchorFrame = CreateFrame("Frame", "HandyNotes_NPCs_SearchWindowANCHOR", UIParent, "BackdropTemplate")
	...
	window.f = f
	f:SetScript("OnVerticalScroll", function(self, offset) FauxScrollFrame_OnVerticalScroll(self, offset, currentLINE_HEIGHT, update) end)
	f:SetAllPoints(true)
	...
	anchorFrame:Hide()
	tinsert(UISpecialFrames, anchorFrame:GetName()) 
...
function Search:SearchNPCs()
	...
	for k,v in pairs(data["items"]) do
		if v.icon and string.find(v.name:lower(), text:lower(), 1, true) then 
...
function Search:DumpRecipesForZone(zone)
	...
	for coord, npc in pairs(data["nodes"][zone]) do
		if npc.npcID and data["vendors"][npc.npcID] then
			for item in data["vendors"][npc.npcID]:gmatch("([^,]+)") do
				item = tonumber(item)
				if data["items"][item] and data["items"][item].teaches then 
	...
	self:UpdateHeader(L["Recipes"], L["Cost"]) 
	update(self.window.f)
end
...
local xB = 0
local yB = 0
function Search:DragStart(frame, button) 
